...
const navbarlinks = [
  ...
  { id: 2, title: 'Sobre Nosotros', link: '/SobreNosotros' },
  { id: 3, title: 'Contacto', link: '/Contacto' },
];

const navbarRedes = [
  { id: 1, title: 'Instagram', link: '/', icon: 'bi bi-instagram' },
  { id: 2, title: 'Whatsapp', link: '/', icon: 'bi bi-whatsapp' },
];
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 bg-lime-400 w-full bg-opacity-30 backdrop-blur-md">
        ...
        <div className="hidden md:block">
          <ul className="flex sm:space-x-48 space-x-4">
            {navbarlinks.map((link) => (
              <li key={link.id}>
                <Link
                  to={link.link} // Usamos Link en lugar de <a>
          ...
          <ul className="flex space-x-4">
            {navbarRedes.map((link) => (
              <li key={link.id}>
                ...
                >
                  <i
                    className={`${link.icon} sm:text-2xl text-lg text-white hover:text-gray-900 transition-all duration-300`}
                  ></i>
                </a>
              </li>
            ))}
      ...
      {/* Menu móvil */}
      <div
        className={`md:hidden absolute w-full bg-lime-700 transition-all duration-300 ${
        ...
        <ul className="flex space-x-4 py-2 border-t border-gray-900 justify-center">
          {navbarRedes.map((link) => (
            <li key={link.id}>
              ...
              >
                <i
                  className={`${link.icon} text-lg text-white hover:text-gray-950`}
                ></i>
              </a>
            </li>
          ))}
